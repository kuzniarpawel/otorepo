...
export const PARAMETRY = {
  toneL: {
    ...
    granicaPl: 'ZMIERZONA GRANICA MODELU: powyżej ok. 167 Hz aferent nasyca się przy prędkości pchnięcia głową i vHIT zaczyna wychodzić patologicznie MIMO sprawnego kanału. Faza drażnienia w chorobie Ménière’a to ok. 150 Hz — i tam vHIT zostaje prawidłowy, co jest jej cechą odróżniającą.',
    granicaEn: 'A MEASURED LIMIT OF THE MODEL: above about 167 Hz the afferent saturates at head-impulse velocity and vHIT starts coming out abnormal DESPITE an intact canal. The irritative phase of Ménière’s disease is about 150 Hz — and there vHIT stays normal, which is its distinguishing feature.',
  },
  toneR: {
    ...
    granicaPl: 'ZMIERZONA GRANICA MODELU: powyżej ok. 167 Hz aferent nasyca się przy prędkości pchnięcia głową i vHIT wychodzi patologicznie mimo sprawnego kanału. Faza drażnienia Ménière’a (ok. 150 Hz) zostawia vHIT prawidłowy — nie przesuwaj suwaka do końca, żeby ją zobaczyć.',
    granicaEn: 'A MEASURED LIMIT OF THE MODEL: above about 167 Hz the afferent saturates at head-impulse velocity and vHIT comes out abnormal despite an intact canal. The irritative phase of Ménière’s (about 150 Hz) leaves vHIT normal — do not push the slider to the end to see it.',
  },
  gainL: {
    ...
    granicaPl: 'ZMIERZONA GRANICA MODELU: ten suwak jest wzmocnieniem TRANSDUKCJI kanału, a nie odczytem badania. Gain ZMIERZONY w pchnięciu zależy dodatkowo od tonu (nasycenie aferentu przy 300 Hz): przy tonie 200 Hz i suwaku 1,0 pchnięcie wychodzi 0,63 i patologicznie. Sam silnik odwzorowuje PEŁNE wypadnięcie jako 0,35, a nie 0 — zero jest krańcem modelu.',
    granicaEn: 'A MEASURED LIMIT OF THE MODEL: this slider is the canal’s TRANSDUCTION gain, not a test reading. The gain MEASURED on a head impulse also depends on tone (afferent saturation at 300 Hz): with tone at 200 Hz and the slider at 1.0 the impulse comes out 0.63 and abnormal. The engine itself represents a COMPLETE loss as 0.35, not 0 — zero is a limit of the model.',
  ...
  toneAcL: {
    ...
    granicaPl: 'ZMIERZONA GRANICA MODELU: izolowane wypadnięcie kanału PIONOWEGO daje tu równocześnie lokalizację obwodową (gałąź nerwu) i wpis „izolowany oczopląs pionowo-skrętny — klasycznie podejrzany o ośrodek". To sprzeczność w silniku, nie w Twoim rozumowaniu: zestaw HINTS liczy oczopląs wyłącznie ze składowej POZIOMEJ i nie widzi pionowej.',
    granicaEn: 'A MEASURED LIMIT OF THE MODEL: an isolated VERTICAL canal loss here yields both a peripheral localization (a nerve branch) and the entry "isolated vertical-torsional nystagmus — classically suspicious for a central cause". That is a contradiction in the engine, not in your reasoning: the HINTS battery computes the nystagmus from the HORIZONTAL component only and does not see the vertical one.',
  },
  toneAcR: {
  ...
  sacculeL: {
    ...
    granicaPl: 'ZMIERZONA GRANICA MODELU: odczyt VEMP jest ASYMETRIĄ między uszami. Wyzeruj obie strony, a zdanie o VEMP ZNIKNIE z opisu badania — tak samo jak w rzeczywistości milczy wskaźnik asymetrii przy utracie obustronnej. Próba kaloryczna ma na to osobną bramkę (mówi wprost o ubytku obustronnym), VEMP w tym silniku jej nie ma.',
    granicaEn: 'A MEASURED LIMIT OF THE MODEL: the VEMP reading is an ASYMMETRY between the ears. Zero both sides and the VEMP sentence DISAPPEARS from the findings — just as an asymmetry ratio falls silent in a real bilateral loss. The caloric test has a separate gate for this (it states a bilateral deficit outright); VEMP in this engine does not.',
  },
  sacculeR: {
