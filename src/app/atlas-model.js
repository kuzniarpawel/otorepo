...
export const ATLAS = [
  ...
  },
  {
    ...
    kryteria: [
      {
        ...
        punkty: [
          ...
          { litera: "C", pl: "Obustronnie obniżona lub nieobecna czynność kątowego VOR, udokumentowana przez: obustronnie patologiczny gain poziomego kątowego VOR < 0,6 mierzony metodą video-HIT albo techniką cewki twardówkowej, I/LUB obniżoną odpowiedź kaloryczną (suma bitermicznych maksymalnych szczytowych SPV PO KAŻDEJ STRONIE OSOBNO < 6°/s), I/LUB obniżony gain poziomego kątowego VOR < 0,1 przy stymulacji sinusoidalnej na fotelu obrotowym (0,1 Hz, Vmax = 50°/s) ORAZ przesunięcie fazy > 68 stopni (stała czasowa < 5 s).", en: "Angular VOR function bilaterally reduced or absent, evidenced by: horizontal angular VOR gain pathological on BOTH sides at < 0.6 on video-HIT or by scleral coil, AND/OR a diminished caloric response (bithermal maximum peak SPV summed ON EACH SIDE SEPARATELY < 6°/s), AND/OR horizontal angular VOR gain < 0.1 on sinusoidal rotatory-chair stimulation (0.1 Hz, Vmax = 50°/s) AND ALSO a phase lead > 68 degrees (time constant < 5 s)." },
    ...
    progi: [
      ...
      { ranga: "kryterium", wartosc: "< 0,1 przy 0,1 Hz, Vmax = 50°/s", wielkoscPl: "gain poziomego kątowego VOR na fotelu obrotowym", wielkoscEn: "horizontal angular VOR gain on the rotatory chair", kontekstPl: "kryterium C, opcja trzecia — w LINII KRYTERIÓW połączona z przesunięciem fazy spójnikiem ORAZ", kontekstEn: "criterion C, third option — joined to the phase lead by AND in the CRITERIA LINE" },
      { ranga: "kryterium", wartosc: "> 68 stopni (stała czasowa < 5 s)", wielkoscPl: "przesunięcie fazy (phase lead) na fotelu obrotowym", wielkoscEn: "phase lead on the rotatory chair", kontekstPl: "kryterium C, opcja trzecia; liczba 68 nie ma w dokumencie żadnego wyprowadzenia ani noty", kontekstEn: "criterion C, third option; the figure 68 has no derivation or note anywhere in the document" },
      { ranga: "kryterium", wartosc: "< 5 s (wartość typowa dla BVP, w kryterium C-3); < 10 s (wartość uznawana za nieprawidłową, w komentarzu)", wielkoscPl: "stała czasowa odpowiedzi VOR", wielkoscEn: "VOR response time constant", kontekstPl: "kryterium C-3 używa 5 s, komentarz o fotelu obrotowym stawia próg patologii na 10 s — to nie są progi konkurencyjne, ale łatwo je pomylić", kontekstEn: "criterion C-3 uses 5 s while the rotational-testing commentary sets the pathological threshold at 10 s — not competing thresholds, but easily confused" },
    ...
    granicePl: [
      "DO TEJ PRACY UKAZAŁA SIĘ ERRATA (J Vestib Res 2023;33(1):87), której TREŚCI posiadany pełny tekst NIE ZAWIERA — jest tylko przypis o jej istnieniu. Nie wiadomo więc, czy któryś z poniższych progów został po 2017 r. skorygowany. Przed użyciem liczb jako twardych stałych erratę należy sprawdzić osobno.",
      "PRÓG FOTELA OBROTOWEGO MA W JEDNEJ PRACY TRZY BRZMIENIA. Kryterium C-3: gain < 0,1 przy 0,1 Hz i 50°/s ORAZ phase lead > 68° (koniunkcja). Abstrakt: te same liczby, ale spójnik „i/lub” (wystarczy jedno). Nota o badaniach obrotowych: gain < 0,15 przy 0,05–0,1 Hz i 60°/s. To najpoważniejszy rozjazd w dokumencie i najbardziej prawdopodobny kandydat na treść erraty — ale potwierdzić się tego z samej pracy nie da.",
    ...
    graniceEn: [
      "AN ERRATUM TO THIS PAPER EXISTS (J Vestib Res 2023;33(1):87) whose CONTENT is NOT in the full text held — only a footnote recording its existence. It is therefore unknown whether any threshold below was corrected after 2017. Before using these figures as hard constants the erratum must be checked separately.",
      "THE ROTATORY-CHAIR THRESHOLD HAS THREE WORDINGS IN ONE PAPER. Criterion C-3: gain < 0.1 at 0.1 Hz and 50°/s AND a phase lead > 68° (conjunction). The abstract: the same figures but joined by „and/or” (either suffices). The note on rotational testing: gain < 0.15 at 0.05–0.1 Hz and 60°/s. This is the paper's most serious internal divergence and the most likely candidate for the content of the erratum — but that cannot be confirmed from the paper itself.",
  ...
  },
  {
    ...
    zespol: "nd",
    wSilniku: "poza-zakresem",
    wSilnikuDowod: "W worktree C:/Users/kuzni/OneDrive/Dokumenty/Otorepo_code/files/.claude/worktrees/atlas-otoneurologiczny: grep -rniE 'cervical dizz|cervicogenic|zawroty szyjne|zawrotow szyjnych|zawrotów szyjnych' src/ = 0 trafień. Jednostka nie jest w programie ani nazwana, ani niesiona kryteriami. Kark ISTNIEJE w silniku, ale jako KINEMATYKA, nie jako jednostka: grep -rniE 'neckPitch|neckYaw|provokeNeck|stepNeck|NECK_PREFIX' src/ = 28 trafień w dwóch plikach (src/engine/vestibular.js, src/pose/maneuvers.js) — to ramię od osi obrotu do błędnika zależne od kąta karku i pozy prowokacyjne manewrów, czyli geometria głowy na tułowiu, a nie hipoteza szyjnego pochodzenia zawrotów. Kontrola: grep -rniE 'szyj' src/ = 13 trafień, wszystkie o czym innym (cVEMP szyjny, człon szyjny dźwigni, pogranicze czaszkowo-szyjne, ból karku jako pozycja w kwalifikacji wstępnej w src/app/triage-model.js:179); grep -rni 'propriocep' src/ = 3 trafienia.",
    ...
    kryteria: [
      ...
      },
      {
        ...
        punkty: [
          ...
          { litera: "2", pl: "Opcja rozstrzygająca: potwierdzić, czy objawy są wyzwalane także wtedy, gdy głowa jest utrzymywana NIERUCHOMO WZGLĘDEM ZIEMI, a tułów obracany pod nieruchomą głową — bo to jest ta konfiguracja dynamiczna, w której zachodzi prawdziwy ruch szyi bez żadnego ruchu głowy.", en: "Decisive option: check whether the symptoms come on as well when the head is held STILL RELATIVE TO THE EARTH while the trunk is turned underneath it — that arrangement being the one in which the neck genuinely moves and the head does not." },
