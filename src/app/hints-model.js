...
export const OSTRZEZENIE = {
  ...
  trescPl: 'Wymagana pilna ocena neurologiczna. Obrazowanie: MRI z dyfuzją (DWI) i angiografią (MRA); tomografia komputerowa NIE nadaje się do wykluczenia udaru tylnego dołu, ale JEST ZALECANA do wykrycia KRWOTOKU lub innych patologii (angio-TK rozpozna zwężenie tętnicy kręgowej lub podstawnej — [H59] Strupp 2022). Wczesne MRI bywa fałszywie ujemne — DWI jest fałszywie ujemne u 12–50% chorych w pierwszych 48 h (przy udarach ≤ 10 mm nawet 53%), więc przy prawidłowym pierwszym badaniu i utrzymującym się podejrzeniu potrzebna jest ocena SERYJNA; odstępu powtórzenia źródło nie podaje — [H58] Kim 2022.',
  trescEn: 'Urgent neurological evaluation is required. Imaging: MRI with diffusion (DWI) and angiography (MRA); CT is NOT suitable for ruling out a posterior-fossa stroke, but it IS RECOMMENDED to detect a HAEMORRHAGE or other pathology (CT angiography diagnoses vertebral or basilar artery stenosis — [H59] Strupp 2022). Early MRI can be false-negative — DWI is falsely negative in 12–50% within the first 48 h (up to 53% for strokes ≤ 10 mm), so if the first scan is normal and suspicion persists, SERIAL evaluation is required; the source gives no repeat interval — [H58] Kim 2022.',
};
