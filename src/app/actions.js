...
import { MANEUVERS, CANALS, sizedSeconds, CANAL_OF, DIAG, ACT_STEPS, actTimeline, sessionSim, bltInit } from '../pose/maneuvers.js';
...
function toggleChmura(){ state.ensemble = !state.ensemble; render(); }
...
/* ZASIEW SESJI ZE SCENARIUSZA (V19). Scenariusz historii pozycyjnej JEST aktem otwierajacym:
   `bltInit` symuluje te historie i zwraca polozenie zloga, wiec przy wlaczonej sesji klik
   scenariusza nie „ustawia opcji", tylko ZASIEWA lancuch od tego stanu.
   RESTART, NIE NADPISANIE: podmiana phi przy zachowanych starych aktach zlamalaby inwariant
   „lancuch ≡ jedna nic symulacji" — historia i akty musza pochodzic z jednego przebiegu.
   TRZY PULAPKI (zmierzone po stronie main, przeniesione swiadomie):
     · bondFrac = settled ? 1 : 0. Bond 0 dla `neutral` dawalby FALSZYWY wynik lying-down 0,077 —
       zlog „swiezo przemieszczony" tam, gdzie scenariusz mowi „dlugi siad".
     · ZERO readhesion za siad historii: `settled:false` to konwencja karty, nie stan do „poprawienia".
     · synchronizacja `state.bltScenario`, zeby wylaczenie sesji wracalo do zasianej historii,
       a nie do domyslnego „textbook". */
function seedSessionFromScenario(k){
  state.bltScenario = k;
  if(!state.session){ render(); return; }
  const ini = bltInit(state.side || "P", k) || {};
  state.session = { canal:"horizontal", side:state.side || "P", size:state.size || "medium",
    phi: ini.phi0 != null ? ini.phi0 : null,
    xi: 0,                                   // kontrakt karty: scenariusz daje POLOZENIE, nie ogon
    bondFrac: ini.settled ? 1 : 0,
    exited: !!ini.exitedInHistory, rep: 0, acts: [{ kind:"historia", scen:k }],
    aktDomkniety: null };
  render();
}
