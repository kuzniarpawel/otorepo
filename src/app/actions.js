...
import { otworzWpis, zamknijWpis as zamknijWpisAtlasu, ustawZespol as ustawZespolWpisow, ustawZakres as ustawZakresWpisow, ustawSzukaj as ustawSzukajWpisow, wyczyscFiltry as wyczyscFiltryWpisow } from './atlas-state.js';
...
const HINTS_PRESETS = {
  ...
  // D8/V22: pacjent t-EVS (BPPV) na ekranie HINTS — NeuroVOR-owo ZDROWY (cała patologia BPPV żyje
  // w silniku Vestibular / tevsDemoSim); lekcja: HINTS niestosowalny, wykonaj Dix. Wpis PRZED "rd" —
  // kolejność w OBIEKCIE jest nośna: ostatni preset pętli grabów snapshotu zostaje bazą kluczy
  // hints/nerve/* w golden (rd wnosi comp=0.85 — tevs na końcu przestawiłby 4 klucze nerve).
  tevs:    { get label(){return t("Pacjent t-EVS (BPPV) — demo","t-EVS patient (BPPV) — demo");},       make:()=>({}) },
...
function setHintsGrace(open){ state.hintsGrace=!!open; }         // D8/V22: karta GRACE-3 (details) — jak wyżej, bez re-renderu
function setHintsVmCrit(open){ state.hintsVmCrit=!!open; }       // V28: karta kryteriów migreny przedsionkowej (details) — jak wyżej, bez re-renderu
...
function pickSide(s){
  ...
  // D1/V10 (main): strona jest TOŻSAMOŚCIĄ złogu — zmiana strony przy włączonej sesji zaczyna nowy złóg,
  // a nie przenosi stary do drugiego ucha.
  if(state.session && state.side!==s) state.session=freshSession(state.session.canal, s, state.session.size);
...
function resetTestLocal(k){
  ...
  if(innaProba){
    ...
    // D4/V16 (main): mechanizm w obrębie fenotypu jest stanem dydaktycznym KARTY, nie pacjenta —
    // przy zmianie próby wraca do klasycznego, jak wariant.
...
function setVariant(v){ markDecision(state,"variant",v); state.variantZrodlo="nadpisany"; state.mechanism=null;   // flip = wybór FENOTYPU → mechanizm wraca do klasycznego (D4/V16, higiena kart)
  if(state.testKey) markSeen(state,"interpretSeen"); render(); }
...
function repeatDixProvoke(){ markSeen(state,"obsSeen");
  if(state.session){ sessionProvoke(); return; }
...
function resetDixProvoke(){ markSeen(state,"obsSeen"); state.dixRep=0;
  if(state.session){ resetSession(); return; }
  render(); }

/* ---- z gałęzi main (ocena II): mechanizm D4, sesja ciągła D1/V10+V19, chmura D9/V20, egzamin D7/V21 ---- */
...
function commitAct(kind, timeline, restSecs){
  ...
  // wpis aktu niesie CZAS (karty sesji z main) ORAZ położenie/wyjście (karty przebiegu z futureUI) —
  // jeden zapis dla obu pokoleń ekranów, zamiast dwóch list aktów o różnych polach.
  S.acts.push({kind, t:Math.round(dur), phi: S.phi!=null ? Math.round(S.phi) : null, exited: !!S.exited});
  S.tSession+=dur;   // render() u WYWOŁUJĄCEGO — sessionProvoke inkrementuje rep PO commicie, render musi widzieć stan końcowy
}
...
function seedSessionFromScenario(k){
  state.bltScenario=k;                                     // ZAWSZE: sesja OFF też czyta scenariusz (karta B&L), a sesja ON wróci do zasianej historii
  const S=state.session;
  if(!S || !state.testKey || !SCEN_DRIVEN.has(state.testKey)
     || mechOf(state.variant, state.mechanism, DIAG[state.testKey].canal)!=="canalo"){ render(); return; }
...
function pickSize(s){ if(state.size===s) return; state.size=s;
  if(state.session) state.session=freshSession(state.session.canal, state.session.side, s);   // bond/dynamika skalują się z r — łańcuch między rozmiarami to footgun (D1/V10)
...
function setGuideSide(s){
  ...
  if(state.session) state.session=freshSession(state.session.canal, s, state.session.size);   // strona = tożsamość złogu (D1/V10)
  state.side=s;
  /* Od 2026-08-17 to JEDYNE miejsce, w którym użytkownik deklaruje ucho (selektor strony zniknął
     z ekranu eksperckiego jako duplikat). Bez tej linii `sideZrodlo` zostawałoby na null nawet po
     jawnym przestawieniu pigułki, a karta doboru i generator opisu twierdziłyby, że strona jest
     wartością domyślną — czyli aplikacja przestałaby przyznawać się do tego, co użytkownik zrobił. */
  state.sideZrodlo="wybrany";
...
function setDiagSide(s){ if(state.side===s) return;
  if(state.session) state.session=freshSession(state.session.canal, s, state.session.size);   // strona = tożsamość złogu (D1/V10)
  markDecision(state,"side",s); state.dixRep=0; render(); }
...
/* CHMURA ZLOGU (D9/V20) — TRYB WIDOKU, nie stan pacjenta ani fizyki. Kanoniczna POJEDYNCZA
   symulacja zostaje zrodlem timerow, wedrowki otolitu, sesji i objawow; chmura jest wylacznie
   drugim SPOJRZENIEM na ten sam manewr. Dlatego zmiana strony ani rozmiaru go nie resetuje. */
/* SCALENIE (Etap 3): oba przełączniki delegują do implementacji z gałęzi main. futureUI miał
   własne, UBOŻSZE kopie (sesja bez `stuck`/`inCrus`/`tSession` i bez re-adhezji za spoczynek),
   a dwie kopie tego samego stanu fizycznego to dokładnie ten rodzaj rozjazdu, który wododział
   B&L już raz kosztował. Nazwy zostają OBIE, bo wołają je dwa różne pokolenia ekranów. */
function toggleChmura(){ toggleEnsembleMode(!state.ensemble); }
function przelaczSesje(){ toggleSessionMode(!state.session); }
// Zasiew scenariusza żyje wyżej, w bloku sesji ciągłej z main (wersja V19: akty + tSession).
// UWAGA (zostawione bez zmiany przy scaleniu): `markDecision(state,"blt",…)` pisze do pola `blt`,
// którego w state.js NIE MA — bije wyłącznie licznik decyzji. Samo `bltScenario` ustawia
// `seedSessionFromScenario`. Poprawka nazwy pola przesunęłaby moment bicia licznika, więc jest
// osobnym krokiem po scaleniu, nie w nim.
function setBltScenario(k){ if(state.bltScenario===k && !state.session) return; markDecision(state,"blt",k); seedSessionFromScenario(k); }
...
function startManeuver(key, side){
  ...
  // D1/V10: kanał (i strona) to tożsamość złogu — manewr innego kanału zaczyna nowy złóg
  if(state.session && (CANAL_OF[key]!==state.session.canal || s!==state.session.side)) state.session=freshSession(CANAL_OF[key], s, state.size);
...
function domknijAkt(){
  ...
  /* SCALENIE (Etap 3): fizykę aktu liczy `commitAct` z gałęzi main — ta sama funkcja, przez którą
     idą prowokacja, manewr i przerwa. futureUI miał tu WŁASNĄ, krótszą wersję tego samego kroku,
     bez re-adhezji za spoczynek (readhesion), bez `stuck`/`inCrus` i bez zegara aktowego, więc ten
     sam złóg przesuwał się inaczej w zależności od tego, KTÓRY przycisk go ruszył. Zostaje wyłącznie
     to, co w futureUI było własne i słuszne: MOMENT domknięcia (przejście do interpretacji)
     i idempotencja po (próba, strona, numer prowokacji). */
  try { commitAct(proba, actTimeline(proba, state.side), SESSION_REST); } catch(e){ return; }
  S.rep = (S.rep||0) + 1;
  S.aktDomkniety = klucz;
}
...
/* ============ ATLAS OTONEUROLOGICZNY (E6) ============
   Wszystkie akcje przechodzą przez `atlas-state.js`, który pisze WYŁĄCZNIE do pól `atlas*`.
   Żadna nie dotyka `state.triage`, `state.obs`, `state.flow` ani pól symulatora — i to jest cała
   treść decyzji użytkownika z 2026-08-22: ścieżka ostra zostaje OGÓLNA, jednostki szczegółowe
   mieszkają osobno. Wejście do atlasu ma NICZEGO nie zmienić w toczącym się badaniu; wyrocznia
   bierze odcisk tamtych pól przed wejściem i po wyjściu.

   ATLAS NIE JEST WEJŚCIEM DO BADANIA. Nie ma tu ani jednej funkcji ustawiającej `mode` na 'hints'
   albo 'treat' — bramka `wolnoBadac` zostaje jedynymi drzwiami modułu HINTS, a `sciezkaDozwolona`
   jedynym warunkiem wejścia w ścieżkę. */
function goAtlas(){
  state.area="atlas"; state.mode="atlas"; state.screen="atlasLista";
  zamknijWpisAtlasu(state);
  render();
}
/* `skad` niesie NAWIGACJĘ, nie treść: wpis wygląda tak samo niezależnie od wejścia. Klinicysta,
   którego kwalifikacja odesłała do atlasu, ma wrócić do SWOJEGO werdyktu, a nie na listę
   osiemnastu jednostek — inaczej link z kwalifikacji kosztowałby go zgubienie kontekstu. */
function otworzWpisAtlasu(klucz, skad){
  if(!otworzWpis(state, klucz, skad === undefined ? null : skad)){ render(); return; }
  state.area="atlas"; state.mode="atlas"; state.screen="atlasWpis";
  render();
}
function wrocZWpisuAtlasu(){
  // Powrót do kwalifikacji NIE czyści odpowiedzi — `openTriage` ich nie rusza, a `syncTriage`
  // tylko odbija stan w pasku przebiegu. Gdyby czyścił, link z werdyktu byłby pułapką.
  if(state.atlasSkad === 'triage'){ state.atlasSkad=null; zamknijWpisAtlasu(state); openTriage(); return; }
  zamknijWpisAtlasu(state); state.screen="atlasLista"; render();
}
function ustawZespolAtlasu(z){ ustawZespolWpisow(state, z === 'null' ? null : z); render(); }
function ustawZakresAtlasu(s){ ustawZakresWpisow(state, s === 'null' ? null : s); render(); }
function ustawSzukajAtlasu(f){ if(ustawSzukajWpisow(state, f)) render(); }
function wyczyscFiltryAtlasu(){ wyczyscFiltryWpisow(state); render(); }
...
function setLangUI(lang){
  ...
  syncLangBar(); syncSessionBar(); render();   // syncSessionBar: etykiety sesji/chmury/egzaminu też przez t() (z main)
...
Object.assign(window, { goAtlas, otworzWpisAtlasu, wrocZWpisuAtlasu, ustawZespolAtlasu, ustawZakresAtlasu, ustawSzukajAtlasu, wyczyscFiltryAtlasu, schowajAktualizacje, wdrozAktualizacjeTeraz, goOpis, przelaczSekcjeOpisu, edytujOpis, ustawEdycjeOpisu, wrocDoWyliczonego, kopiujOpis, eksportujOpis, zapiszSesjeOpisu, usunSesjeOpisu, usunWszystkieSesjeOpisu, przywrocSesjeOpisu, wczytajSesjeOpisu, ustawTolerancjeKontroli, goLab, otworzEksperymentLab, wrocDoEksperymentow, ustawStanowiskoLab, przelaczPorownanieLab, ustawParametrLab, resetLab, opisParametruLab, goNauka, otworzPrzypadek, wrocDoBiblioteki, ustawFiltrNauki, goEtapNauki, odpowiedzNauki, wskazowkaNauki, zakonczPrzypadek, wyczyscPostepNauki, wczytajPostepNauki, goHintsKwal, ustawPrzeszkolenieHints, pomijajKwalifikacje, cofnijPominiecie, zacznijBadanieHints, otworzSymulatorHints, otworzLaboratorium, ustawSkladowaHints, ustawPowodNiewiarHints, goHintsKrok, dalejHints, wsteczHints, pokazWynikHints, wrocDoBadaniaHints, wyczyscBadanieHints, biezacyKrok, goKontrola, wrocDoManewru, ustawWynikKontroli, ustawPowodKontroli, kontrolaAkcja, kontrolaAlternatywa, powtorzManewrKontroli, pytajOZakonczeniu, zakonczSesje, potwierdzPrzerwe, zmienManewr, ustawTrybCzasu, zakonczSerie, goInterpret, przyjmijMechanizm, nadpiszMechanizm, wrocDoWyprowadzonego, idzDoProby, togglePorownanie, goObs, wrocDoProby, setObsWystapil, setObsPole, oznaczObsPole, setObsPowod, setObsGrupa, wyczyscObs, przyjmijObs, toggleVizPause, setVizSpeed, vizStepFwd, resetViz, openTriage, setTriage, toggleTriageFlaga, goTriageStep, resetTriage, triageGo, setHintsPlane, hintsHIT, rerunHintsHIT, setMode, openHints, setHintsDx, setHintsNeuritisSide, setHintsFix, setHintsGaze, setHintsComp, setHintsRecovery, hintsActivePatient, loadHintsPreset, loadHintsNeuritis, openHintsCustom, exitHintsCustom, setHintsAdvanced, findParamSpec, fmtParamVal, setHintsParam, applyHintsNerve, setHintsNerveEar, setHintsNerveBranch, setHintsNerveSev, hintsRandomPatient, revealHintsQuiz, hintsSCDSStim, hintsCustomDiff, hintsEncode, hintsDecode, saveShareHints, loadHintsFromHash, loadHintsFromStore, pickSide, pickCanal, pickMan, pickTest, openMan, openTest, setDixObs, toggleDiagCentral, setVariant, repeatDixProvoke, resetDixProvoke, genPlan, pickSize, setGuideSide, setDiagSide, setBltScenario, przelaczSesje, domknijAkt, toggleChmura, seedSessionFromScenario, startPlan, startManeuver, startDiag, backToSetup, goStep, toggleAuto, toggleSound, setView3d, setLangUI, syncLangBar, toggleNeuroOverlay, setHintsGrace, setHintsVmCrit, setMechanism, toggleSessionMode, resetSession, sessionProvoke, sessionManeuver, sessionRest, syncSessionBar, toggleEnsembleMode, syncEnsembleBar, examStart, newExamPatient, examReveal, examEnd, toggleExamMode, syncExamBar });
