  ...
  $("#app").innerHTML=`
    ...
    ${(state.step === n-1 && !state.session) ? (()=>{
      ...
      return `<div class="panelbox" style="margin-bottom:12px"><h4>${t("Chmura złogu","Debris cloud")}
        ...
        ${konwersja ? "" : `<div class="note">${t("Kontekst kliniczny: skuteczność repozycji to ok. 80 % po pojedynczej sesji, z dalszym wzrostem przy powtórzeniach ([H27] AAO-HNS 2017); przegląd [H29] Cochrane 2014 daje ustąpienie zawrotu 56 % vs 21 % bez manewru. To NIE są te same miary co frakcja powyżej — model pokazuje masę złogu, nie odsetek pacjentów.","Clinical context: repositioning succeeds in about 80 % after a single session, rising with repeats ([H27] AAO-HNS 2017); the review [H29] Cochrane 2014 reports vertigo resolution 56 % vs 21 % without a maneuver. These are NOT the same measures as the fraction above — the model shows debris mass, not a percentage of patients.")}</div>`}
