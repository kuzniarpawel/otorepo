...
function epley(side){
  ...
  return {name:t("Manewr Epleya","Epley maneuver"),canal:"posterior",side,headCamera:"topDownBehind",steps:[
    {title:t("Pozycja wyjściowa","Starting position"),body:"sit",yaw:aY,face:"fwd",seconds:null,progress:0.02,
     headSlot:{kind:"backTurn",dir:A}, headText:t(`Stań za pacjentem i skręć głowę ${sideN(A,"przysl")}.`,`Stand behind the patient and turn the head ${A==="L"?"to the left":"to the right"}.`),
     instr:t(`Pacjent siada na kozetce. Obróć jego głowę o 45° w stronę chorą (${sideN(A,"bier")}).`,`The patient sits on the couch. Turn their head 45° toward the affected side (${sideN(A)}).`)},
    {title:t("Szybkie położenie na plecach","Quick move to supine"),body:"supineHang",yaw:aY,face:"up",seconds:30,progress:0.18,
     instr:t(`Połóż pacjenta szybko na plecach, głowa odchylona ~20° poza krawędź kozetki, wciąż obrócona 45° w stronę chorą (${sideN(A,"bier")}). Utrzymaj do ustąpienia oczopląsu.`,`Lay the patient down quickly on their back, head extended ~20° beyond the edge of the couch, still turned 45° toward the ${sideN(A)} side. Hold until the nystagmus subsides.`)},
    {title:t("Obrót głowy o 90°","Turn the head 90°"),body:"supineHang",yaw:hY,face:"up",seconds:30,progress:0.45,
     instr:t(`Obróć głowę o 90° w stronę zdrową, tak że jest odchylona 45° w stronę zdrową (${sideN(H,"bier")}). Utrzymaj.`,`Turn the head 90° toward the healthy side, so it is rotated 45° toward the ${sideN(H)} side. Hold.`)},
    {title:t("Obrót na bok zdrowy","Roll onto the healthy side"),body:sideH,yaw:hY,face:"down",seconds:30,progress:0.74,
     instr:t(`Obróć pacjenta na bok ${sideN(H,"bierM")} i dodatkowo głowę o kolejne 90°, tak by nos był skierowany ku podłodze. Utrzymaj.`,`Roll the patient onto the ${sideN(H)} side and turn the head a further 90°, so the nose points toward the floor. Hold.`)},
...
function semont(side){
  ...
  return {name:t("Manewr Semonta","Semont maneuver"),canal:"posterior",side,steps:[
    {title:t("Pozycja wyjściowa","Starting position"),body:"sitFront",yaw:hY,face:"fwd",seconds:null,progress:0.02,
     instr:t(`Pacjent siedzi na środku kozetki, twarzą do badającego. Obróć jego twarz o 45° w stronę zdrową (${sideN(H,"bier")}).`,`The patient sits in the middle of the couch, facing the examiner. Turn their face 45° toward the healthy side (${sideN(H)}).`)},
    {title:t("Szybki rzut na bok chory","Rapid drop onto the affected side"),body:leanA,yaw:hY,face:"up",seconds:90,progress:0.25,
     instr:t(`Szybko połóż pacjenta na bok chory (${sideN(A,"bierM")}). Głowa pozostaje obrócona — nos ku górze. Utrzymaj 1–3 min.`,`Quickly lay the patient onto the affected side (${sideN(A)}). The head stays turned — nose pointing up. Hold 1–3 min.`)},
    {title:t("Szybki rzut na bok przeciwny","Rapid drop onto the opposite side"),body:leanH,yaw:hY,face:"down",seconds:90,progress:0.72,
     instr:t(`Bez zmiany ustawienia głowy szybko przemieść pacjenta na bok przeciwny (${sideN(H,"bierM")}) — nos ku podłodze. Utrzymaj 1–3 min.`,`Without changing the head position, quickly move the patient onto the opposite side (${sideN(H)}) — nose toward the floor. Hold 1–3 min.`)},
...
function bascule(side){
  ...
  return {name:t("Manewr Bascule","Bascule maneuver"),canal:"posterior",side,mechanism:"cupulo",steps:[   // mechanism:"cupulo" → wędrówka pokazuje przyleganie/odklejanie od osklepka i wyjście do łagiewki
    ...
    {title:t("Szybko na bok zdrowy — nos w dół","Quickly onto the healthy side — nose down"),body:leanH,yaw:hY,face:"down",seconds:30,progress:0.28,
     instr:t(`Szybko połóż pacjenta na bok zdrowy (${sideN(H,"bierM")}); głowa wciąż obrócona, nos skierowany ku podłodze. Utrzymaj 15–30 s.`,`Quickly lay the patient onto the healthy side (${sideN(H)}); the head still turned, nose pointing toward the floor. Hold 15–30 s.`)},
    {title:t("Przerzut na bok chory — nos w górę","Swing onto the affected side — nose up"),body:leanA,yaw:hY,face:"up",seconds:30,progress:0.52,
     instr:t(`Szybkim ruchem przerzuć głowę i tułów RAZEM o 180° na bok chory (${sideN(A,"bierM")}) — nos ku sufitowi. Utrzymaj 15–30 s.`,`With a swift motion, swing the head and trunk TOGETHER 180° onto the affected side (${sideN(A)}) — nose toward the ceiling. Hold 15–30 s.`)},
...
function lempert(side){
  ...
  return {name:t("Manewr Lemperta (rolka BBQ)","Lempert maneuver (BBQ roll)"),canal:"horizontal",side,headCamera:"topDownBehind",steps:[
    {title:t("Na plecach, głowa ku choremu","Supine, head toward the affected side"),body:"supineFlat",yaw:yawH,face:"up",seconds:30,progress:0.08,
     instr:t(`Pacjent leży na plecach. Obróć głowę o 90° w stronę chorą (${sideN(A,"bier")}). Utrzymaj.`,`The patient lies supine. Turn the head 90° toward the affected side (${sideN(A)}). Hold.`)},
    ...
    {title:t("Obrót na bok zdrowy","Roll onto the healthy side"),body:sideH,yaw:-aY,face:"fwd",seconds:30,progress:0.52,
     instr:t(`Obróć głowę i ciało o kolejne 90° w stronę zdrową (${sideN(H,"bier")}). Utrzymaj.`,`Turn the head and body a further 90° toward the healthy side (${sideN(H)}). Hold.`)},
    ...
    {title:t("Obrót na bok chory","Roll onto the affected side"),body:sideA,yaw:aY,face:"fwd",seconds:30,progress:0.92,
     instr:t(`Obróć o kolejne 90° na bok chory (${sideN(A,"bierM")}). Utrzymaj.`,`Roll a further 90° onto the affected side (${sideN(A)}). Hold.`)},
...
function gufoniGeo(side){
  ...
  return {name:t("Manewr Gufoniego (geotropowy)","Gufoni maneuver (geotropic)"),canal:"horizontal",side,headCamera:"topDownBehind",steps:[
    ...
    {title:t("Szybko na bok zdrowy","Quickly onto the healthy side"),body:leanH,yaw:0,face:"fwd",seconds:60,progress:0.32,   // bok ZDROWY w dół, widok od przodu; głowa w linii ciała (leanX|fwd = ta sama grawitacja co sideX|fwd)
     instr:t(`Szybko połóż pacjenta na bok zdrowy (${sideN(H,"bierM")}). Oczopląs geotropowy ku podłodze (ku uchu ${sideN(H,"cel")}). Utrzymaj 1–2 min, do ustąpienia oczopląsu.`,`Quickly lay the patient onto the healthy side (${sideN(H)}). Geotropic nystagmus toward the floor (toward the ${sideN(H)} ear). Hold 1–2 min, until the nystagmus subsides.`)},
...
function gufoniApo(side){
  ...
  return {name:t("Manewr Gufoniego (apogeotropowy)","Gufoni maneuver (apogeotropic)"),canal:"horizontal",side,mechanism:"cupulo",headCamera:"topDownBehind",steps:[   // apogeotropowy HC-BPPV = KUPULOLITIAZA: złóg startuje WBITY w osklepek (krok 1)
    ...
    {title:t("Szybko na bok chory","Quickly onto the affected side"),body:leanA,yaw:0,face:"fwd",seconds:60,progress:0.30,   // bok CHORY w dół, widok od przodu; głowa w linii ciała (leanX|fwd → gHead ±x, ta sama fizyka co sideX|fwd)
     instr:t(`Szybko połóż pacjenta na bok chory (${sideN(A,"bierM")}). Oczopląs apogeotropowy ku górze (ku uchu ${sideN(H,"cel")}). Utrzymaj 1–2 min.`,`Quickly lay the patient onto the affected side (${sideN(A)}). Apogeotropic nystagmus upward (toward the ${sideN(H)} ear). Hold 1–2 min.`)},
...
function zuma(side){
  ...
  return {name:t("Manewr Zuma e Maia","Zuma e Maia maneuver"),canal:"horizontal",side,mechanism:"cupulo",headCamera:"topDownBehind",steps:[   // mechanism:"cupulo" → narracja przylegania (krok 1) i odklejania (krok 2)
    ...
    {title:t("Szybki dekubit na bok chory","Rapid decubitus onto the affected side"),body:sideA,yaw:0,face:"fwd",seconds:180,progress:0.25,
     instr:t(`SZYBKO połóż pacjenta na bok chory (${sideN(A,"bierM")}) — rzut bezwładnościowy odrywa złóg od osklepka. Głowa w linii ciała. Utrzymaj ~3 min.`,`QUICKLY lay the patient onto the affected side (${sideN(A)}) — the inertial jolt detaches the debris from the cupula. Head in line with the body. Hold ~3 min.`)},
    ...
    {title:t("Obrót na plecy, głowa ku zdrowemu","Roll supine, head toward the healthy side"),body:"supineFlat",yaw:yawH,face:"up",seconds:180,progress:0.65,
     instr:t(`Obróć ciało do leżenia na plecach; głowa pozostaje skręcona 90° ku stronie zdrowej (${sideN(H,"dop")}). Utrzymaj ~3 min.`,`Roll the body to lie supine; the head stays rotated 90° toward the healthy side (${sideN(H)}). Hold ~3 min.`)},
    {title:t("Lekkie przygięcie głowy","Slight head flexion"),body:"supineFlex",yaw:yawH,face:"up",seconds:180,progress:0.85,
     /* V25 (main): kąt przygięcia z anatomii atlasu (HC_TILT_TXT ≈ 10°), nie podręcznikowe 30° — patrz R3. */
...
function kim(side){
  ...
  return {name:t("Manewr Kima (CRM)","Kim maneuver (CRM)"),canal:"horizontal",side,mechanism:"cupulo",headCamera:"topDownBehind",steps:[
    ...
    {title:t("Na bok chory, nos 45° ku podłodze","Onto the affected side, nose 45° toward the floor"),body:sideA,yaw:yawD,face:"fwd",seconds:120,progress:0.28,
     instr:t(`Obróć pacjenta na bok chory (${sideN(A,"bierM")}) i skręć głowę tak, aby nos był 45° ku podłodze. Tu protokół stosuje WIBRACJĘ wyrostka sutkowatego (~30 s) — odrywa złóg od osklepka; model jej nie liczy (granica modelu). Utrzymaj ~2 min.`,`Turn the patient onto the affected side (${sideN(A)}) and rotate the head so the nose points 45° toward the floor. Here the protocol applies MASTOID VIBRATION (~30 s) — it detaches the debris from the cupula; the model does not compute it (a model boundary). Hold ~2 min.`)},
    ...
    {title:t("Na bok zdrowy","Onto the healthy side"),body:sideH,yaw:0,face:"fwd",seconds:120,progress:0.78,
     instr:t(`Obróć pacjenta na bok zdrowy (${sideN(H,"bierM")}), głowa w linii ciała. Utrzymaj ~2 min.`,`Turn the patient onto the healthy side (${sideN(H)}), head in line with the body. Hold ~2 min.`)},
...
// kupulolitiaza = uporczywy (trzyma się, dopóki pozycja utrzymana).
  /* OKNA 55/75 s ZAMIAST 40/60 — wartosci z galezi futureUI, przeniesione PRZEZ POMIAR, nie z sentymentu.
     ...
     czastki obcinalaby przebieg po cichu. Zanik trwa do 39,9 s (kanal poziomy), wiec okno musi go zmiescic
     z zapasem: 55 s. Okno uporczywe 75 s > progu 60 s z kryteriow Barany — przy braku zaniku tEnd JEST oknem,
     wiec liczba musi prog PRZEKRACZAC, a nie go dotykac (main mial 60 -> tEnd 60,50 s).
     ZOSTAJE OTWARTE: kanal przedni. Main poszerzyl jego okno do 70 s, bo napad AC trwa w silniku ~61 s
     (szczyt dopiero ~25 s) i wspolne okno ucinalo animacje w pol napadu. Skutek: postac PRZEMIJAJACA
     kanalu przedniego trwa 61,25 s, czyli WIECEJ niz 60 s, ktore obiecuje jej wlasny chip. Tego nie da sie
     naprawic szerokoscia okna — to dlugosc napadu w modelu kloci sie z progiem Barany. Decyzja kliniczna.
     SCALENIE futureUI→main (Etap 3): okna 55/75 zostają (mają za sobą pomiar barany:check), a kark
     toru diagnostycznego z V24 (PROVOKE_POSE/NECK_PREFIX, wyżej) wchodzi razem z nimi — to dwie
     niezależne warstwy: okno mówi JAK DŁUGO patrzymy, kark JAKA SIŁA działa na złóg. */
function engineXi(canal, side, persistent, q, init, mech, neck){
  ...
  // Okno PRZEDNIEGO 75 s (ocena II, A6/V8 + pomiar barany:check): napad AC trwa w silniku ~61 s
  // (szczyt dopiero ~25 s) — wspólne okno 40 s ucinało animację przy 36% szczytu, w pół napadu.
  // Zmienia wyłącznie tEnd animacji diagnostyki (manewrów nie zasila — tam manStepEnv).
  ...
  // Etap 3 (scalenie): tHold z futureUI (55/75, pomiar barany:check), kark z main (V24).
  const nv = q==null ? provokeNeck(canal, side) : (neck||null);
  const seg={q: q||provokeQ(canal,side), tTrans:0.5, tHold: (persistent||canal==="anterior")?75:55, pivot:"body"};
...
/* ===== V25: ZGIĘCIE POZYCJI TESTU ROLL — WYPROWADZONE Z ATLASU, NIE WPISANE Z PODRĘCZNIKA =====
   ...
   pomiary porównawcze [H17] 19.9°, Wu 17.4°). Dlatego poza bierze kąt STĄD, a nie z podręcznika.
   DWIE LICZBY, NIE JEDNA (korekta po krytyce geometrycznej — pierwsza wersja myliła je ze sobą):
     HC_TILT_DEG = acos(|n_y|/|n|) = 10.4398° — PEŁNE (dwuścienne) nachylenie płaszczyzny kanału.
       To wielkość PORÓWNYWALNA Z PIŚMIENNICTWEM: Della Santina [H17] mierzy dokładnie ją (kąt
...
const HC_TILT_DEG = Math.acos(Math.abs(_HCN[1])/Math.hypot(_HCN[0],_HCN[1],_HCN[2])) * 180/Math.PI;  // 10.4398° — do porównań z [H17]
...
/* ============ Dynamika manewru per KROK (przeniesione z render/svg-screens.js — Blok 10) ============
   Te funkcje NIGDY nie były renderowaniem: nie dotykają DOM, nie czytają `state`, liczą wyłącznie
   fizykę złogu z silnika. Siedziały w module renderującym, przez co nie dało się ich zawołać z
   wyroczni w gołym Node — a to na nich stoi cała odpowiedź „ile trwa oczopląs tego kroku" i „w
   którym kroku złóg opuszcza kanał", czyli dwie liczby, które Blok 10 pokazuje klinicyście.
   Jedyna, która ZOSTAŁA w renderze, to `currentManSim()` — bo czyta cache w `state`. */

// symulacja + podział na segmenty czasowe odpowiadające krokom planu
...
/* φ→ułamek ścieżki. WYJŚCIE LICZY SIĘ Z KANAŁU, nie ze stałej 178° (poprawione 2026-08-14).
   Do oceny II łuk był wspólny i kończył się na 178°; dziś ARC_SPAN jest ZMIERZONY per kanał
   (poziomy 267,3° · przedni 281,1° · tylny 318,8° — IE-Map, R1), więc próg 178° pokazywałby złóg
   „w łagiewce" już na 56–67 % łuku. Gałąź, która tego używa (manewr BEZ ekspulsji, mechanizm inny
   niż kupulo), jest dziś NIEOSIĄGALNA — zmierzone 0 z 54 przypadków (manewr × strona × rozmiar) —
   więc to poprawka miny, nie usterki: gdyby któryś manewr przestał czyścić kanał, UI kłamałoby
   o położeniu złogu. Brak `canal` → dawne 178° (zgodność wsteczna dla re-eksportu). */
const phiToFrac = (phi, canal) => {
  const luk = (canal && Vestibular.ARC_SPAN && Vestibular.ARC_SPAN[canal]) || 178;
  return Math.max(0, Math.min(1, phi/luk));
};
...
function manFractions(man, plan){
  ...
  if(plan.mechanism==="cupulo" && !man.exited){   // GUFONI APO (konwersja apo→geo): złóg WBITY w osklepek (krok1) → ODKLEJA się (krok2) →
    return {fr: plan.steps.map((_,i)=> i===0?0.04 : i===1?0.34 : 0.72), exitStep:-1, schemat:true};   // WĘDRUJE do pozycji GEOTROPOWEJ w kanale (krok3) i tam zostaje (bez wyjścia do łagiewki).
  }
  if(!man.exited){          // KONWERSJA nie-kupulityczna: ruch wg φ z silnika, złóg NIE wychodzi do łagiewki.
    return {fr: plan.steps.map((_,i)=>phiToFrac(manPhi(man,i,1), plan.canal)), exitStep:-1, schemat:true};   // Bascule (kupulo) WYCHODZI (man.exited) → nie tu, tylko rampa niżej.
  }
  ...
  // `schemat` mówi, CZY krok kuracyjny wynika z FIZYKI (physExit), czy z reguły n-2. Blok 10 pokazuje
  // klinicyście „kryterium przejścia dalej" i nie wolno mu podawać schematu jako pomiaru.
  return {fr, exitStep:cure, schemat: !(physExit>=1)};
}
...
const DIAG={
  dix:{ get name(){return t("Manewr Dix–Hallpike","Dix–Hallpike test");}, get tests(){return t("kanał tylny","posterior canal");}, canal:"posterior",
    ...
    latNote:(A,v)=> v==="canalo"
      ? t(`Postać klasyczna (kanalolitiaza): złóg swobodny w kanale tylnym po stronie ${sideN(A,"dop")}.`,`Classic form (canalithiasis): free-floating debris in the posterior canal on the ${sideN(A)} side.`)
  ...
  roll:{ get name(){return t("Test pozycyjny (Roll / Pagnini–McClure)","Positional test (Roll / Pagnini–McClure)");}, get tests(){return t("kanał poziomy","horizontal canal");}, canal:"horizontal",
    ...
    get poseNote(){return t(`Uwaga do pozycji: zgięcie karku to ~${HC_TILT_TXT}°, a nie podręcznikowe „20–30°" — bo przy tym kącie grawitacja kładzie się DOKŁADNIE w płaszczyźnie kanału bocznego tego atlasu. To ten sam cel, którym klinika uzasadnia swój kąt („unieś głowę, żeby kanał boczny stanął pionowo"), tyle że policzony dla anatomii ZMIERZONEJ (19,9° · 17,4° · ${HC_TILT_DEG.toFixed(1).replace(".",",")}°), a nie dla kanonicznych 30° — te przyszły z konwencji kalorycznej i nie są pomiarem czułości testu. Pełny test obustronny bywa w modelu ZABIEGIEM: wyprowadza złóg do łagiewki, więc kontrolny test może być niemy. Klinicznie to nie błąd — pamiętaj tylko, że ujemna kontrola zaraz po prowokacji nie dowodzi wyleczenia sprzed badania. (Liczby, granice i zależność od holdu — engine_doc, wpisy R3 i V25.)`,`A note on the position: the neck flexion is ~${HC_TILT_TXT}°, not the textbook "20-30°" — at this angle gravity comes to lie EXACTLY in the plane of this atlas's lateral canal. It is the same goal the clinic uses to justify its own angle ("raise the head so the lateral canal stands vertical"), only computed for MEASURED anatomy (19.9° · 17.4° · ${HC_TILT_DEG.toFixed(1)}°) rather than for the canonical 30°, which came from the caloric convention and is not a measurement of the test's sensitivity. In this model the full bilateral test can act as a TREATMENT: it carries the debris into the utricle, so the control test may be mute. Clinically that is not an error — just remember that a negative control right after the provocation does not prove the patient was already cured. (Figures, limits and the dependence on hold time — engine_doc, entries R3 and V25.)`);},
    features:featsByVariant,
    latNote:(A,v,mech)=>{
    ...
    phases:(A,v,scen,mech)=>{ const H=otherSide(A), geo=(v==="canalo");
      ...
      const mk=down=>{ const up=otherSide(down);
        ...
        return {...pose(down),
          ...
          label: mech==="light" ? t(`geotropowy TRWAŁY — ku uchu w dole (${sideN(down,"cel")})`,`PERSISTENT geotropic — toward the lower ear (${sideN(down)})`)
               : geo ? t(`geotropowy — ku uchu w dole (${sideN(down,"cel")})`,`geotropic — toward the lower ear (${sideN(down)})`) : t(`apogeotropowy — ku uchu w górze (${sideN(up,"cel")})`,`apogeotropic — toward the upper ear (${sideN(up)})`),
  ...
  headhang:{ get name(){return t("Test deep head-hang","Deep head-hang test");}, get tests(){return t("kanał przedni","anterior canal");}, canal:"anterior",
    ...
    features:featsByVariant,
    latNote:(A,v)=> v==="canalo"
      ? t(`Kanalolitiaza kanału przedniego: oczopląs ku dołowi (downbeat) ze SKŁADOWĄ SKRĘTNĄ ku uchu choremu — w tej geometrii 0,74 pionu. UWAGA: model pokazuje torsję, bo taką ma geometria kanału, ale U CHOREGO jej często NIE MA (brak torsji u 42% pozycyjnych downbeatów i u 57% potwierdzonych AC-BPPV — [H31]), a przeciwstronna kupulolitiaza tylna wygląda tak samo ([H32]). Dlatego lateralizacja oczopląsem POZOSTAJE NIEWIARYGODNA — stronę różnicuj reakcją na manewr i kontekstem klinicznym.`,`Anterior-canal canalithiasis: downward nystagmus (downbeat) with a TORSIONAL component toward the affected ear — 0.74 of the vertical in this geometry. NOTE: the model shows the torsion because that is the canal's geometry, but IN THE PATIENT it is often absent (no torsion in 42% of positional downbeats and in 57% of confirmed AC-BPPV — [H31]), and a contralateral posterior cupulolithiasis looks the same ([H32]). Lateralization by nystagmus therefore REMAINS UNRELIABLE — differentiate the side by the response to the maneuver and the clinical context.`)
    ...
    phases:(A,v)=>[{
      ...
      label:t(`ku dołowi + skrętny ku uchu choremu (torsja 0,74 pionu — u chorego bywa nieobecna)`,`downward + torsional toward the affected ear (torsion 0.74 of the vertical — often absent in patients)`),
      note: v==="canalo"
        ? t("BEZ istotnej latencji (złóg startuje dociśnięty do osklepka — wyprowadzenie R7): downbeat ze skrętem ku uchu choremu, narasta WOLNO (szczyt ~25 s), wygasa ~1 min, wyczerpuje się przy powtórzeniu. Oczopląsu nie używaj do ustalenia strony: model rysuje torsję z geometrii kanału, ale u chorego bywa ona śladowa albo nieobecna (57% potwierdzonych AC — [H31]).","WITHOUT significant latency (the debris starts pressed against the cupula — the R7 derivation): downbeat with torsion toward the affected ear, builds SLOWLY (peak ~25 s), fades by ~1 min, fatigues on repetition. Do not use the nystagmus to establish the side: the model draws the torsion from the canal's geometry, but in patients it is often a trace or absent (57% of confirmed AC — [H31]).")
...
function recommend(testKey,variant,mech){
  ...
  if(testKey==="headhang"){
    return variant==="canalo"
      ? {primary:"yacovino",alts:[],note:t("Kanalolitiaza kanału przedniego — manewr Yacovino (deep head-hang → szybki ruch brody do klatki). Kanał przedni jest rzadki; oczopląs to downbeat ze skrętem ku uchu choremu, ale strony nim NIE ustalisz — u chorego torsja bywa nieobecna, a przeciwstronna kupulolitiaza tylna wygląda tak samo; różnicuj kontekstem i reakcją na manewr.","Anterior-canal canalithiasis — the Yacovino maneuver (deep head-hang → quick chin-to-chest movement). The anterior canal is rare; the nystagmus is a downbeat with torsion toward the affected ear, but you still cannot establish the side by nystagmus, differentiate by context and response to the maneuver.")}
  ...
  if(antMode || canal==="anterior")
    ...
    return { ...(variant==="cupulo" ? poza : emg), subtype: variant==="cupulo"
      ...
      /* DWA TWIERDZENIA O TYM SAMYM — ZESPOJONE 2026-08-15 (futureUI), utrzymane przy scaleniu.
         ...
         mierzy napad ~61 s — czyli WIĘCEJ niż minuta. Trzy miejsca, dwie wersje prawdy; klinicysta
         widzi obie na jednym ekranie. Bierzemy więc oba pola WPROST z `featsByVariant`, tego samego
         źródła, które zasila chipy — i sprzeczność przestaje być możliwa strukturalnie, a nie przez
         czujność. KIERUNEK zostaje z V26 (main): maska kliniczna anterior.t zdjęta, więc karta mówi
         o torsji tyle, ile mówi geometria, z zastrzeżeniem 42/57% braku torsji u chorych ([H31]). */
...
export { HC_TILT_DEG, HC_FLEX_DEG, HC_TILT_TXT, SIDE, stepPivot, otherSide, earToScreen, yawToA, makeManualOrientation, epley, semont, bascule, lempert, yacovino, gufoniGeo, gufoniApo, zuma, kim, MANEUVERS, CANALS, XI_CARD, BLT_HISTORY, bltInit, bltPhases, bltZones, bltDirWord, ldtPhases, nullScan, nullYawOf, SCEN_DRIVEN, TAU_BOND, readhesion, SESSION_REST, SIT_SEG, ACT_STEPS, PHASE_OF, actTimeline, sessionInit, sessionSim, sessionPreview, nysFromGeom, nysFromDyn, provokeQ, engineXi, xiEnvelope, POSE_SPEC, poseOf, headQOf, stepGravity, stepHeadQ, composeHead, SK, SKEL, fkJoints, POSE3D, TORSO_Q, bodyClass, bodyJoints, poseSpec, gravArrowFor, sizeRadius, holdMult, sizedSeconds, derivedHold, maneuverTimeline, maneuverSim, ENS_GRID, ensembleSim, featsByVariant, DIAG, variantLabels, MECHS_BY_PHENO, mechOf, variantOfMech, persistentOf, SHORT_PHI0, rollShortPhases, mechLabels, recommend, baranyClassify, CANAL_OF, PRIORS, mulberry32, randomPatient, TEST_OF_CANAL, examPhaseNys, examAnswerKey, TEVS_REST, tevsDemoSim, JAM_DEMO, jamDemo, PROVOKE_POSE, poseNeck, provokeNeck, NECK_PREFIX,
         /* z futureUI (Blok 10): odmiana nazwy strony + dynamika manewru per krok wyjęta z renderu */
         SIDE_FORMY, sideN, computeManSim, manStepEnv, stepXiPeak, manPhi, phiToFrac, manExitStep, manFractions, guideNysSeconds };
