...
/* K1f ODWROCONE 2026-08-17 (decyzja uzytkownika): strona ma DOKLADNIE JEDNO miejsce w aplikacji —
   pigulke L/P w naglowku karty manewru. Selektor na ekranie eksperckim byl jej duplikatem i to on
   rodzil rozjazd `state.side` ↔ `plan.side`, ktory Blok 10 gasil `patchManeuverSide` i strazikiem
   w `setGuideSide`. Bramka pilnuje dzis zdania PRZECIWNEGO niz dotad: ekran wyboru NIE MA wlasnego
   selektora strony, a karta manewru go MA i dziala. Kierunek „nie ma" jest tu wazniejszy niz
   wyglada: duplikat wroci najlatwiej przez copy-paste sasiedniej grupy `mechOpt`. */
T('K1f/strona-bez-duplikatu-na-wyborze', !/onclick="pickSide\('[LP]'\)"/.test(app()),
  'ekran eksperckiego wyboru nie moze miec wlasnego selektora strony — jedynym miejscem jest pigulka na karcie manewru');
A('openMan')('lempert');
T('K1f2/strona-na-karcie-manewru', /onclick="setGuideSide\('L'\)"/.test(app()) && /onclick="setGuideSide\('P'\)"/.test(app()),
  'karta manewru musi miec pigulke L/P — po usunieciu selektora to jedyne wejscie do strony');
A('setGuideSide')('L');
eq('K1f3/pigulka-ustawia-strone', [st.side, st.plan && st.plan.side, st.sideZrodlo], ['L', 'L', 'wybrany']);
czysty(); A('pickCanal')('horizontal'); A('setVariant')('canalo');
         ...
         z powodem, nie zmyslony akt. */
{
  ...
  /* AK4 ODWROCONE PRZY SCALENIU DO 1bcd9e3 (V19). Do tej wersji Bow & Lean NIE MIAL wpisu
     ...
     AK4b PRZELICZONE PRZY SCALENIU main→futureUI (2026-08-17). Zdanie „akt Bow & Lean opróżnia
     kanał (phi 267°)" bylo prawdziwe przy KONWENCJI futureUI, gdzie wlaczenie sesji tworzylo zlog
     ze wspolczynnikiem wiazania 0 — czyli SWIEZO PRZEMIESZCZONY. Scalona sesja pochodzi z main
     (`freshSession`): zlog SPOCZYWA z pelna adhezja (bond 1, phi = restPhi 199,8°), a naped sklonu
     0,026 nie zrywa wiazania (fStat 0,04). ZMIERZONE na scalonym silniku, akt bowlean/P:
       spoczynek (freshSession)      phi 199,8 — BEZ RUCHU
       bond 0 (dawna konwencja)      phi 267,3 EXIT
       zasiew textbook (phi0 146°)   phi  74,4
       zasiew afterDix               phi 184,4
       zasiew afterRoll (phi0 220°)  phi 267,3 EXIT
     Czyli R10 nie znikl — przeniosl sie tam, gdzie ma desygnat: opróżnienie kanału zalezy od tego,
     gdzie zlog byl PRZED proba (wododzial 190°), a nie od samego wykonania skłonu. Bramka pilnuje
     dzis OBU polowek tego zdania, bo kazda z nich moze pekac osobno. */
  czysty();
  A('openTest')('bowlean');
  A('przelaczSesje')();
  A('goInterpret')();
  ...
  T('AK4b/spoczynek-jest-niemy', ost.exited !== true && Math.abs(ost.phi - 200) < 1.5,
    `zlog w spoczynku z pelna adhezja ma zostac na miejscu (phi ~199,8, bez wyjscia) — zmierzone: ${JSON.stringify(ost)}`);
  czysty();
  A('openTest')('bowlean');
  A('przelaczSesje')();
  A('setBltScenario')('afterRoll');
  A('goInterpret')();
  const ostAR = (st.session.acts || []).slice(-1)[0] || {};
  T('AK4c/po-afterRoll-wyprowadza', ostAR.exited === true && st.session.exited === true,
    `R10 na zywo: po zasiewie afterRoll (phi0 ~220° za wododzialem) akt Bow & Lean ma OPROZNIC kanal — zmierzone: ${JSON.stringify(ostAR)}`);
...
/* ═══════════ O. LICZNOŚĆ ═══════════ */
const OCZEKIWANE = 99;   /* 96 + 3: scalenie main->futureUI 2026-08-17 — K1f rozbite na trzy (brak duplikatu strony na wyborze + pigulka na karcie manewru + jej skutek na stan) i AK4c (po zasiewie afterRoll akt B&L oproznia kanal); AK4b zmienilo TRESC: spoczynek z pelna adhezja jest NIEMY */   /* 94 + 2: AK4b (akt B&L wyprowadza) + AK5 (kazda proba ma akt) */   /* 88 + 6: AK1-AK4 (granica aktu: przejscie do interpretacji przesuwa zlog) */   /* 84 + 4: MC0-MC3 (sesja ciagla znosi mnoznik meczliwosci) */   /* 77 + 7: N1 x5 prob + N2 + N3 (lista prob wyliczana z DIAG) */
if (bledy.length) {
